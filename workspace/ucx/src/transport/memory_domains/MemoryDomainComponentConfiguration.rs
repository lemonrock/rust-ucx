...
{
	#[inline(always)]
	fn read_from_environment(name: &CStr, environment_variable_prefix: &CStr) -> Result<Self, ErrorCode>
	{
		let mut this = MemoryDomainComponentConfiguration(null_mut());
		
		let unsupported_file_name = null_mut();
		
		let status = unsafe { uct_md_config_read(name.as_ptr(), environment_variable_prefix.as_ptr(), unsupported_file_name, &mut this.0) };
